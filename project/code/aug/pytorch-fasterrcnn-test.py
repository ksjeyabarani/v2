...
lrs = str(lr).replace(".", "")
momentums = str(momentum).replace(".", "")
decays = str(decay).replace(".", "")
print("lr={0}, num_epochs={1}, momentum={2}, decay={3}".format(lr, num_epochs, momentum, decay))
...
WEIGHTS_FILE = "{0}/fasterrcnn_resnet50_fpn_aug_epoch_{1}_{2}_{3}_{4}.pth".format(MODEL_DIR, num_epochs, lrs, momentums, decays)
...
print("Running evaluation on {0}".format(WEIGHTS_FILE))
if not os.path.exists(WEIGHTS_FILE):
  print("Model file {0} does not exist".format(WEIGHTS_FILE))
  sys.exit(-1)
...
print("Finished running evaluation on {0} with {1} images".format(WEIGHTS_FILE, num_images))
